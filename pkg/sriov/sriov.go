	...
	if err = tempNS.Do(func(linkNS ns.NetNS) error {
		...
		defer func() {
			if err != nil {
				...
				_ = netlink.LinkSetNsFd(tempNSLinkObj, int(initns.Fd()))
			}
		}()
		...
		defer func() {
			if err != nil && tempNSLinkObj != nil {
				_ = s.nLink.LinkSetName(tempNSLinkObj, linkName)
			}
		}()
		...
		if err = netns.Do(func(_ ns.NetNS) error {
			...
			defer func() {
				if err != nil {
					initns, _ := ns.GetCurrentNS()
					_ = s.nLink.LinkSetNsFd(tempNSLinkObj, int(initns.Fd()))
				}
			}()
...
func (s *sriovManager) ReleaseVF(conf *sriovtypes.NetConf, podifName string, netns ns.NetNS) error {
	...
	return netns.Do(func(_ ns.NetNS) error {
		...
		err = tempNS.Do(func(hostNS ns.NetNS) error {
			...
			defer func() {
				if err != nil {
					_ = s.nLink.LinkSetNsFd(tempNSLinkObj, int(netns.Fd()))
				}
			}()
			...
			defer func() {
				if err != nil {
					_ = s.nLink.LinkSetName(tempNSLinkObj, podifName)
				}
			}()
