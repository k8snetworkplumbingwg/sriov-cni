	...
	if err = tempNS.Do(func(linkNS ns.NetNS) error {
		...
		defer func() {
			if err != nil {
				...
				if tempNSLinkObj, err := s.nLink.LinkByName(linkName); err == nil {
					linkSetNsFdError := netlink.LinkSetNsFd(tempNSLinkObj, int(initns.Fd()))
		...
		if err = netns.Do(func(_ ns.NetNS) error {
			...
			defer func() {
				if err != nil {
					if netNSLinkObj, err := s.nLink.LinkByName(podifName); err == nil {
